...
for id in range(len(list_cap)):
    ...
    while list_cap[id].isOpened():
        ...
        for i, box in enumerate(license_plate_boxes):
            ...
            directory_path = f'{result_folder}\{list_filename[id]}'
            if not os.path.exists(directory_path):
                os.makedirs(directory_path, exist_ok=True)

            plate_filename = f'{directory_path}\license_plate_{frame_id}_{i}.jpg'
            ...
            if results:
                data = {"ID": i+1, "Path": plate_filename, "Time": frame_id, 'License plate': results[0][1]}
                print("Data:",data)
